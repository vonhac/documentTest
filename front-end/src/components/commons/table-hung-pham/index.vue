        ...
        <template slot="items" slot-scope="props">
          <tr :class="computedDecorateRow(props.item)">
            <td
              ...
              :key="col.value"
              :style="emptyField(col, props.item[col.value])"
              :class="[
...
export default {
  ...
  watch: {
    value() {
      ...
      if (this.options.getTableData != undefined && this.options.getTableData) {
        this.options.getTableData(this.data_table);
      }
      this.filtering();
      this.loadCheckbox();
    },
    ...
    selected: {
      handler() {
        if (
          this.options.disabledLoadCombobox != undefined &&
          this.options.disabledLoadCombobox
        ) {
          return;
        }
        this.loadCheckbox();
  ...
  methods: {
    ...
    loadCheckbox(value) {
      if (this.options.initCheckbox != undefined && this.options.initCheckbox) {
        this.map_checkbox = {};
    ...
    selectAll(select) {
      var checkDuplicate = this.options.checkDuplicate;
      if (checkDuplicate == undefined) {
        ...
        if (_.isBoolean(select) && !select) {
          ...
          for (let key in this.map_checkbox) {
            ...
            if (checkValid) {
              this.map_checkbox[key] = this.all_check;
              this.$bus.$emit(`select_${key}_${this.table_id}`, true);
    ...
    selectRow(val, key, isStop, isIgonre) {
      ...
      var configDuplicate = false;
      if (checkDuplicate != undefined) {
        configDuplicate = true;
        checkValid = checkDuplicate(val, key, this.map_checkbox, true);
      }

      if (!configDuplicate || (configDuplicate && checkValid)) {
        ...
        this.$bus.$emit(`select_${key}_${this.table_id}`, false);
      }
    },
    ...
    emptyField(col, val) {
      ...
      if (!_.startsWith(col.value, '$') && _.isNil(val)) {
